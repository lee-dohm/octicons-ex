defmodule Octicons do
  ...
  See [the website][octicons] for an up-to-date reference of all of the
  available icons.
  ...
  @doc """
  Retrieves the attributes of the icon or `nil` if the named icon doesn't
  exist.
  ...
  * `:"aria-label"` Aria label for the SVG tag. When `aria-label` is specified,
    the `aria-hidden` attribute is removed.

  * `:class` CSS class text to add to the classes already present

  * `:height` Height in pixels to render the icon. If only `:height` is
    specified, width is calculated to maintain the aspect ratio.

  * `:width` Width in pixels to render the icon. If only `:width` is specified,
